...
CREATE TABLE IF NOT EXISTS seo_content_performance (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  content_id UUID REFERENCES blog_posts(id) ON DELETE CASCADE,
...
-- Function để update updated_at timestamps
...
-- Views để dễ dàng truy vấn data

-- View tổng quan SEO performance
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE seo_score >= 90) as excellent_count
...
JOIN blog_posts bp ON scp.content_id = bp.id
LEFT JOIN seo_page_metrics spm ON spm.page_url LIKE '%' || bp.slug || '%'
...
('sabo arena', 1, 2, 8900, 'low', 'up'),
('gaming platform vietnam', 15, 18, 2400, 'medium', 'up'),
('esports vietnam', 8, 12, 5600, 'high', 'up'),
('billiards vietnam', 3, 4, 1800, 'low', 'down'),
('ai gaming automation', 25, 30, 890, 'medium', 'up')
...
COMMENT ON TABLE seo_keyword_rankings IS 'Theo dõi thứ hạng từ khóa trên search engines';
COMMENT ON TABLE seo_page_metrics IS 'Metrics hiệu suất SEO cho từng trang';
COMMENT ON TABLE seo_competitor_analysis IS 'Phân tích competitor và cơ hội';
COMMENT ON TABLE seo_audit_results IS 'Kết quả audit SEO và issues cần fix';
COMMENT ON TABLE seo_content_performance IS 'Hiệu suất content và engagement';
COMMENT ON TABLE seo_backlinks IS 'Theo dõi backlinks và link building';
COMMENT ON TABLE seo_search_console_data IS 'Data từ Google Search Console';

-- Enable RLS (Row Level Security) nếu cần
