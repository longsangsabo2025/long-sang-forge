/**
 * Migration: Add chat_credits table
 * Giới hạn 10 credits/ngày cho mỗi user
 *
 * USAGE: Copy SQL below and run in Supabase Dashboard > SQL Editor
 */
...
const SQL_MIGRATION = `
    ...
    CREATE INDEX IF NOT EXISTS idx_chat_credits_user_date ON chat_credits(user_id, date);

    -- Enable RLS
    ALTER TABLE chat_credits ENABLE ROW LEVEL SECURITY;

    -- Policy: Users can only see their own credits
    DROP POLICY IF EXISTS "Users can view own credits" ON chat_credits;
    CREATE POLICY "Users can view own credits" ON chat_credits
      FOR SELECT USING (auth.uid() = user_id);

    -- Policy: Service role can do everything (for backend)
    DROP POLICY IF EXISTS "Service role full access" ON chat_credits;
    ...
    BEGIN
      ...
      INSERT INTO chat_credits (user_id, date, credits_used, credits_limit)
      VALUES (p_user_id, v_today, 0, 10)
    ...
    -- Function to get current credits (without using)
  ...
  // Execute via Supabase Dashboard SQL or use rpc
  // For now, let's try using a workaround with the REST API
  ...
  try {
    ...
    if (!checkError) {
      console.log("✅ Table chat_credits already exists!");

      // Show current data
      const { data, error } = await supabase.from("chat_credits").select("*").limit(5);
