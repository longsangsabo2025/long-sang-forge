import { getTechIconName } from "@/constants/techIcons";
...
export const TechArchitecture = ({ project }: TechArchitectureProps) => {
  return (
    ...
    >
      <h2 className="font-display text-3xl font-bold mb-6 glow-text">KIẾN TRÚC CÔNG NGHỆ</h2>

      {/* Tech Stack Grid with Real Logos */}
      {project.techStack && (
        <div className="space-y-4">
          <h3 className="font-display text-xl font-semibold text-neon-cyan">Tech Stack</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {project.techStack.map((tech, idx) => (
              ...
              >
                <GlowCard
                  className="p-6 text-center h-full hover:scale-105 transition-transform group"
                  glowColor="cyan"
                >
                  <div className="flex flex-col items-center gap-3">
                    <div className="relative">
                      <Icon
                        icon={tech.iconifyIcon || getTechIconName(tech.name)}
            ...
            <GlowCard glowColor="cyan" className="p-6">
              ...
              <div className="grid grid-cols-4 gap-6">
                {project.technicalDetails.tools.map((tool) => (
                  <div
                    key={tool.name}
                    className="flex flex-col items-center gap-2 p-2 rounded hover:bg-neon-cyan/10 transition-colors group"
                  >
                    <Icon
                      icon={tool.iconifyIcon || getTechIconName(tool.name)}
