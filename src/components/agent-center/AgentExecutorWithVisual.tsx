/**
 * Agent Executor with Visual Execution Steps
 * Enhanced AgentExecutor that shows execution steps on visual canvas
 */
...
interface AgentExecutorWithVisualProps {
  readonly agentId: string;
  readonly agentName: string;
  onExecutionEvent?: (event: ExecutionEvent) => void;
}

export default function AgentExecutorWithVisual({
  agentId,
  agentName,
  onExecutionEvent,
}: AgentExecutorWithVisualProps) {
  ...
  const handleExecute = async () => {
    ...
    if (onExecutionEvent) {
        ...
        plan: {
          steps: [
            ...
            },
            {
              id: 'agent-execution',
              name: 'Execution',
              description: 'Executing agent workflow',
              type: 'execution',
            },
            {
              id: 'agent-completion',
      ...
      onExecutionEvent({ type: 'step_start', stepId: 'agent-planning' });
    }
    ...
    try {
      ...
      if (onExecutionEvent) {
        onExecutionEvent({ type: 'step_complete', stepId: 'agent-planning' });
        onExecutionEvent({ type: 'step_start', stepId: 'agent-execution' });
      }
      ...
      if (onExecutionEvent) {
        onExecutionEvent({ type: 'step_complete', stepId: 'agent-execution' });
        onExecutionEvent({ type: 'step_start', stepId: 'agent-completion' });
      }
      ...
      if (onExecutionEvent) {
        if (executionResult.success) {
          onExecutionEvent({ type: 'step_complete', stepId: 'agent-completion' });
          onExecutionEvent({ type: 'complete' });
        } else {
          onExecutionEvent({
            type: 'step_error',
          ...
          onExecutionEvent({ type: 'error', error: executionResult.error || 'Execution failed' });
        }
      }
      ...
      if (onExecutionEvent) {
        ...
        onExecutionEvent({ type: 'error', error: errorMessage });
      }
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        {/* Execute Button */}
        <Button onClick={handleExecute} disabled={executing || !task.trim()} className="w-full" size="lg">
          {executing ? (
            <>
